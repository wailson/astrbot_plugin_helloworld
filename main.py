...
class MyPlugins(Star):
    ...
    async def show_menu(self, event: AstrMessageEvent):
        """显示功能菜单"""
        try:
            # 统一使用 await event.send 发送消息
            await event.send(event.plain_result(
                "功能菜单：\n1. 成语接龙\n2. 数字累加\n3. 简单问答\n请输入功能编号进入，或者输入“退出”结束。"
            ))
            
            # 移除可能不支持的参数 record_history_chains
            ...
            async def menu_waiter(controller: SessionController, event: AstrMessageEvent):
                ...
                if choice == "1":
                    ...
                    await self.start_idiom_game(controller: SessionController, event)
                elif choice == "2":
                    controller.stop()
                    await event.send(event.plain_result("进入数字累加模式，输入数字，输入“退出”可结束~"))
                    await self.start_number_sum(controller: SessionController, event)
                elif choice == "3":
                    controller.stop()
                    await event.send(event.plain_result("进入简单问答模式，输入问题，输入“退出”可结束~"))
                    await self.start_simple_qa(controller: SessionController, event)
                else:
